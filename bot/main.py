...
async def cmd_start(message: types.Message):
    ...
    print(message.text, message.from_user.id, referral);
        ...
        async with connection.cursor() as cursor:
            ...
            if not isUserExists:
                await cursor.execute('INSERT INTO user (userId, username, profilePicture, registrationDate) VALUES (%s, %s, %s, %s)',
                    (message.from_user.id, message.from_user.full_name, 'https://t.me/i/userpic/320/mUFEw-MTKoo887DrxDGa3xiBpfMb4UCd_hISYR8U0ss.svg', str(time.time())));
