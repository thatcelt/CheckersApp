...
function getValidMovesForPiece(board: Piece[][], row: number, column: number, piece: Piece, capturingOnly: boolean): Position[] {
    ...
    for (const [directionRow, directionColumn] of directions) {
        ...
        const captureCol = c + directionColumn;
        if (r >= 0 && r < BOARD_SIZE && c >= 0 && c < BOARD_SIZE && captureRow >= 0 && captureRow < BOARD_SIZE && captureCol >= 0 && captureCol < BOARD_SIZE && board[r][c] !== Piece.EMPTY && board[r][c] % 2 !== piece % 2 && board[captureRow][captureCol] == Piece.EMPTY)
...
function getValidMovesForKing(board: Piece[][], row: number, column: number, piece: Piece, capturingOnly: boolean): Position[] {
    ...
    for (const [directionRow, directionColumn] of directions) {
        ...
        while (r >= 0 && r < BOARD_SIZE && c >= 0 && c < BOARD_SIZE) {
            if (board[r][c] === Piece.EMPTY && !capturingOnly && !jumped)
                validMoves.push([r, c]);
            else if (board[r][c] % 2 !== piece % 2) {
                const captureRow = r + directionRow;
                const captureColumn = c + directionColumn;

                if (captureRow >= 0 && captureRow < BOARD_SIZE && captureColumn >= 0 && captureColumn < BOARD_SIZE && board[captureRow][captureColumn] == Piece.EMPTY) {
                    captures.push([captureRow, captureColumn]);
                    jumped = true;
                }

                break;
            } else if (board[r][c] !== Piece.EMPTY && board[r][c] % 2 == piece % 2) 
                break;
